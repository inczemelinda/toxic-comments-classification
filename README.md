...
A comparative NLP approach for **toxic comment detection** using the **Jigsaw Toxic Comment Classification Challenge** dataset.  
This project evaluates **TF-IDF + Logistic Regression** baselines and **DistilBERT fine-tuning** for both **binary** and **multi-label** toxicity prediction.

---

## 📖 Overview
Online platforms rely on automated moderation tools that can identify harmful language quickly and consistently.  
...
Data loading & inspection (Kaggle CSV files)

Preprocessing (clean text for TF-IDF, raw text + tokenization for DistilBERT)

Model training (TF-IDF baselines and DistilBERT fine-tuning)

Evaluation (metrics for binary and multi-label tasks)

Saving artifacts (optional, stored locally in models/)
📊 Results
...
Tune multi-label thresholds per label instead of using a single global threshold.

Train DistilBERT on more data and experiment with longer fine-tuning schedules.

Explore alternative transformer models (e.g., RoBERTa) and calibration methods.

Run systematic hyperparameter search for both TF-IDF baselines and transformer training.

📚 References

Kaggle: Jigsaw Toxic Comment Classification Challenge (dataset)
https://www.kaggle.com/competitions/jigsaw-toxic-comment-classification-challenge/data
